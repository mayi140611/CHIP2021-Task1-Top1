...
bert_vocab = transformers.AutoTokenizer.from_pretrained('trueto/medbert-base-chinese')
...
for fold_, (train_ids, dev_ids) in enumerate(kf.split(examples)):
    ...
    bert_config = BertConfig.from_pretrained('trueto/medbert-base-chinese', 
                                             num_labels=len(tm_train_dataset.cat2id))

    dl_module = Bert.from_pretrained('trueto/medbert-base-chinese', 
                                            config=bert_config)
